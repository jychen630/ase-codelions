...
class ToolsApiTest {
  ...
  void getHomeTimeline_missingParams_usesDefaultCount() throws Exception {
    ...
    when(timelineService.getHomeTimeline(eq("test-account"), anyInt()))
        .thenReturn(List.of());

    // Body WITHOUT "params" key at all – should trigger DEFAULT_COUNT branch
    String body = """
        {
          "tool": "get_home_timeline"
        }
    ...
    // Capture the count used
    ArgumentCaptor<Integer> countCaptor = ArgumentCaptor.forClass(Integer.class);
    verify(timelineService)
        .getHomeTimeline(eq("test-account"), countCaptor.capture());

    // ToolsController.DEFAULT_COUNT == 20
    assertEquals(20, countCaptor.getValue());
  }
