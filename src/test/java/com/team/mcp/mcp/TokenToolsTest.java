...
class TokenToolsTest {
  ...
  @Test
  void setToken_missingArgs_returnsError() {
    String t = (String) setTool.call(Map.of()).get(0).get("text");
    assertTrue(t.contains("accountId"));
    assertTrue(t.contains("token"));
  }

  @Test
  void getToken_whenTokenMissing_reportsFalse() {
    // no token stored for acctZ
    var out = getTool.call(Map.of("accountId", "acctZ"));
    String text = (String) out.get(0).get("text");
    assertTrue(text.contains("false"));
  }

  @Test
  void getToken_invalidAccountIdType_returnsError() {
    var out = getTool.call(Map.of("accountId", 123));
    String text = (String) out.get(0).get("text");
    assertTrue(text.contains("error"));
  }

  @Test
  void tokenTools_haveNamesAndDescriptions() {
    assertEquals("set_token", setTool.name());
    assertTrue(setTool.description().contains("Store a token"));

    assertEquals("get_token", getTool.name());
    assertTrue(getTool.description().contains("Report whether"));

    assertEquals("list_tokens", listTool.name());
    assertTrue(listTool.description().contains("List account ids"));
  }
