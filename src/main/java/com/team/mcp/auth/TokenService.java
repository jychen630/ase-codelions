...
/**
 * Minimal in-memory token store for Iteration-1/2.
 *
...
public class TokenService {
  ...
  /**
   * Remove any stored token for an account+provider.
   *
   * @param accountId logical account id
   * @param provider provider id; defaults to {@code "twitter"} if blank
   */
  public void clearToken(final String accountId,
                         final String provider) {
    final String prov = provider == null || provider.isBlank()
        ? "twitter"
        : provider;
    store.remove(key(accountId, prov));
  }
