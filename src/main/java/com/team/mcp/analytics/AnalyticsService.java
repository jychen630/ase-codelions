...
/**
 * Lightweight analytics over the Mastodon/Twitter timeline (or DB).
 *
 ...
 *
 * <p>Iteration 2 adds an ML-style sentiment roll-up using
 * {@link SentimentModel}.
 */
@Service
public class AnalyticsService {
  ...
  /** Client used to obtain tweets (fake or real). */
  ...
  /** ML-style sentiment model (trained/loaded at startup). */
  ...
  public AnalyticsService(
    ...
    // Load/train the small sentiment model once at startup.
    this.sentimentModel = SentimentModel.loadDefault();
  }
  ...
  /**
   * Roll-up summary for dashboards (hashtags + best hour).
   *
   * @param accountId logical account id
  ...
  /**
   * ML-style sentiment roll-up for the account's recent tweets.
   *
   * @param accountId logical account id
   * @return sentiment summary
   */
  public SentimentSummary sentimentSummary(final String accountId) {
    final List<Tweet> tweets = safeFetch(accountId, DEFAULT_FETCH);
    ...
    for (Tweet t : tweets) {
      final SentimentModel.Prediction p = sentimentModel.predict(t.text());
      totalScore += p.score();
      switch (p.label()) {
        case POSITIVE -> pos++;
        case NEGATIVE -> neg++;
    ...
    final int total = tweets.size();
    final double avgScore = total == 0 ? 0.0 : totalScore / total;

    return new SentimentSummary(total, pos, neg, neu, avgScore);
  }
  ...
  /**
   * Sentiment roll-up for dashboards.
   *
   * @param totalTweets number of tweets analysed
   * @param positive count labeled POSITIVE
   * @param negative count labeled NEGATIVE
   * @param neutral count labeled NEUTRAL
   * @param averageScore mean sentiment score
   */
